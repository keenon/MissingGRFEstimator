...
class IMUAnalyze4(AbstractCommand):
    def __init__(self):
        pass

    def register_subcommand(self, subparsers):
        subparser = subparsers.add_parser('imu-analyze4', help='Analyze the raw IMU data.')
        subparser.add_argument('--dataset-home', type=str, default='../data', help='The path to the AddBiomechanics dataset.')

    def run(self, args):
        if 'command' in args and args.command != 'imu-analyze4':
            return False
        ...
        for trial, path in trial_pairs:
            ...
            pose_regularization_synthetic_accelerations = []

            for dof in range(num_dofs):
                raw_poses[dof, :] = filtfilt(b, a, raw_poses[dof, :])

            for pose_regularization in pose_regularizations:
                ...
                pose_regularization_synthetic_accelerations.append(b3d_magnitudes)
            ...
            for i in range(len(pose_regularizations)):
                plt.plot(b3d_times, pose_regularization_synthetic_accelerations[i], label=f'B3D {pose_regularizations[i]}')
            plt.legend()
