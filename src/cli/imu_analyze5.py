...
class IMUAnalyze5(AbstractCommand):
    def __init__(self):
        pass

    def register_subcommand(self, subparsers):
        subparser = subparsers.add_parser('imu-analyze5', help='Visualize the locations of the IMUs on the body in 3D.')
        subparser.add_argument('--dataset-home', type=str, default='../data', help='The path to the AddBiomechanics dataset.')

    def run(self, args):
        if 'command' in args and args.command != 'imu-analyze5':
            return False
        ...
        accelerometers.append((skel.getBodyNode('calcn_r'), nimble.math.Isometry3(np.eye(3), foot_translation)))
        tibia_translation = np.array([0.05, -0.2, 0.0])
        accelerometers.append((skel.getBodyNode('tibia_r'), nimble.math.Isometry3(np.eye(3), tibia_translation)))
        femur_translation = np.array([0.05, -0.2, 0.0])
        accelerometers.append((skel.getBodyNode('femur_r'), nimble.math.Isometry3(np.eye(3), femur_translation)))
        ...
        for trial, path in trial_pairs:
            ...
            timesteps = min(len(csv_time), trial_len)
            while True:
                for i in range(timesteps):
                    skel.setPositions(positions[:, i])
                    gui.nativeAPI().renderSkeleton(skel)

                    for body, transform in accelerometers:
                        world_t = body.getWorldTransform().multiply(transform)

                        pos = world_t.translation()
                        rot = nimble.math.matrixToEulerXYZ(world_t.rotation())
                        gui.nativeAPI().createBox(body.getName()+'_imu', [0.05, 0.05, 0.05], pos, rot, [1.0, 0.0, 0.0, 1.0])
