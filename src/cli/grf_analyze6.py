...
class GRFAnalyze6(AbstractCommand):
    def __init__(self):
        pass

    def register_subcommand(self, subparsers):
        subparser = subparsers.add_parser('grf-analyze6', help='Examine why certain trials have such high residuals that persist after fitting an offset acceleration.')
        subparser.add_argument('--dataset-home', type=str, default='../data', help='The path to the AddBiomechanics dataset.')

    def run(self, args):
        if 'command' in args and args.command != 'grf-analyze6':
            return False
        dataset_home: str = args.dataset_home
        print(f"Running evaluation on {dataset_home}")

        file_trial_pairs = [
            ("/Users/keenonwerling/Desktop/dev/MissingGRFEstimator/data/smoothed/protected/us-west-2:e013a4d2-683d-48b9-bfe5-83a0305caf87/data/Santos2017_Formatted_No_Arm/subject23/subject23.b3d", 0)
        ]
