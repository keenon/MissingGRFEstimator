...
class ThresholdsDetector(AbstractDetector):
    def __init__(self, foot_marker_file: str):
        super().__init__()
        with open(foot_marker_file) as f:
            self.foot_marker_data = json.load(f)

    @staticmethod
    def has_input_outliers(frames: nimble.biomechanics.FrameList) -> bool:
        """
        ...
        for t in range(len(frames)):
            ...
            for marker_name, marker_pos in frame.markerObservations:
            ...
            for force in frame.rawForcePlateForces:
    ...
    def smooth_positions(dt: float, frames: nimble.biomechanics.FrameList) -> Tuple[np.ndarray, np.ndarray]:
        ...
        acc_minimizer = nimble.utils.AccelerationMinimizer(trial_len, acc_weight, regularization_weight, numIterations=500)
    ...
    def get_foot_marker_sets(self, osim: nimble.biomechanics.OpenSimFile) -> List[List[Tuple[nimble.dynamics.BodyNode, np.ndarray]]]:
        """
    ...
    def get_force_weighted_convex_foot_cop_error(skel: nimble.dynamics.Skeleton,
        ...
        num_force_plates = len(frames[0].rawForcePlateForces)
    ...
    @staticmethod
    def get_num_steps(frames: nimble.biomechanics.FrameList) -> Tuple[int, List[int]]:
        num_force_plates = len(frames[0].rawForcePlateForces)
        trial_len = len(frames)
        num_steps = 0
    ...
    def get_foot_travel_distance_in_contact(skel: nimble.dynamics.Skeleton,
                                            ...
                                            positions: np.ndarray,
                                            frames: nimble.biomechanics.FrameList) -> List[float]:
        trial_len = len(frames)
    ...
    def estimate_trial_type(self,
                            ...
                            frames: nimble.biomechanics.FrameList) -> str:
        num_force_plates = len(frames[0].rawForcePlateForces)
        num_steps, _ = self.get_num_steps(frames)
        step_travel_distances = self.get_foot_travel_distance_in_contact(skel, foot_bodies, positions, frames)
    ...
    def estimate_missing_grfs(self, subject: nimble.biomechanics.SubjectOnDisk, trials: List[int]) -> List[List[bool]]:
        ...
        for trial in trials:
            ...
            frames: nimble.biomechanics.FrameList = subject.readFrames(trial, 0, trial_len, includeSensorData=True, includeProcessingPasses=True)
            ...
            if np.mean(subject.getTrialMarkerRMSs(trial, 0)) > 0.08:
                trial_bad = True
            elif self.has_input_outliers(frames):
                trial_bad = True
            if trial_bad:
            ...
            poses, vels = self.smooth_positions(dt, frames)
            ...
            trial_type = self.estimate_trial_type(skel, foot_bodies, poses, vels, frames)
