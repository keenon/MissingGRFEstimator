...
class ThresholdsDetectorPreloaded(AbstractDetector):
    ...
    def estimate_missing_grfs(self, subject: nimble.biomechanics.SubjectOnDisk, trials: List[int]) -> List[List[bool]]:
        ...
        for trial in trials:
            ...
            if self.get_force_weighted_convex_foot_cop_error(skel, foot_markers, poses, raw_force_plate_forces, raw_force_plate_cops, dt) > 0.01:
                result.append([True] * trial_len)
                continue
